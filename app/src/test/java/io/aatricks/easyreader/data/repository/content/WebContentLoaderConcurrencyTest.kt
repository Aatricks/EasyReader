...
class WebContentLoaderConcurrencyTest {

    @Test
    fun `image requests overlap within the host concurrency bound`() = runBlocking {
        ...
        assertTrue(maxConcurrentRequests.get() in 1..imageUrls.size)
        assertTrue(firstRequestAt.get() > 0)
        assertTrue(secondRequestAt.get() > 0)
    }

    @Test
    fun testPrefetchRetryPacing() = runBlocking {
