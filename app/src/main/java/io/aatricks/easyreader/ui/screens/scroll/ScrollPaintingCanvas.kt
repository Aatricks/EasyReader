...
internal const val PARALLAX_FAR = 0.35f
internal const val PARALLAX_MID = 0.6f
internal const val PARALLAX_NEAR = 1f
private const val PARALLAX_SKY = 0.15f
internal const val PARALLAX_BIRDS = 0.4f

// Ridges
internal const val RIDGE_SAMPLE_STEP_DP = 8f
...
// Mist and the dissolve into the end-cap
private const val MIST_TOP_FRACTION = 0.48f
...
/**
 * Draws the emakimono as seen through the viewport: parallax sky, moon, three ridge layers
 * with pines, torii and lanterns, still water with reflections, and the scroll framing.
 * [scrollPx] is the 1x scroll offset; [paintingPx] the world width of the painted journey.
 */
internal fun DrawScope.drawScrollPainting(
    ...
    drawSkyAndStars(palette, scrollPx)
    drawMoon(palette, scrollPx, totalPx)
    drawBirds(palette, scrollPx)
...
private fun DrawScope.drawSkyAndStars(palette: ScrollPalette, scrollPx: Float) {
    drawRect(
    ...
    for (index in firstCell..lastCell) {
        ...
        val alpha = STAR_ALPHA_MIN + random.nextFloat() * STAR_ALPHA_RANGE
        drawCircle(palette.star.copy(alpha = alpha), radius, Offset(x, y))
        if (random.nextFloat() < STAR_BRIGHT_CHANCE) {
            val glint = STAR_GLINT_LENGTH_DP.dp.toPx()
            val stroke = STAR_GLINT_STROKE_DP.dp.toPx()
            val glintColor = palette.star.copy(alpha = STAR_GLINT_ALPHA)
...
private fun DrawScope.drawMoon(palette: ScrollPalette, scrollPx: Float, totalPx: Float) {
    ...
    drawCircle(
        brush = Brush.radialGradient(
            0f to palette.moonGlow.copy(alpha = MOON_GLOW_ALPHA),
            HALF to palette.moonGlow.copy(alpha = MOON_INNER_GLOW_ALPHA),
...
private fun DrawScope.drawRidgeLayer(palette: ScrollPalette, layer: Int, scrollPx: Float) {
    ...
    while (x < size.width + step) {
        path.lineTo(x, ridgeWorldY(layer, x + patternOffset))
        x += step
    }
    ...
    if (layer != LAYER_FAR) {
        drawRidgeDetails(palette, layer, patternOffset)
    }
}

private fun DrawScope.drawMist(palette: ScrollPalette) {
    drawRect(
        brush = Brush.verticalGradient(
